...
export default function DeleteBillModal({ isOpen, onClose, onConfirm, bill }: DeleteBillModalProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [confirmed, setConfirmed] = useState(false);
  const t = useTranslations('purchasing.modals.delete_bill');
    ...
    >
      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900">
                {t('title', { billNumber: bill.billNumber })}
              </h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {/* Bill Details */}
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('details.bill_number')}</h3>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('details.bill_number')}</span>
                <span className="text-sm font-mono font-bold text-slate-900">{bill.billNumber}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('details.amount')}</span>
                <span className="text-sm font-bold text-slate-900">
                  {(bill.totalAmount / 100).toLocaleString()} сўм
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-600">{t('details.status')}</span>
                <span className="text-sm font-bold text-slate-900 uppercase">{bill.status}</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-2">
              <li className="flex items-start gap-2 text-sm text-slate-600">
                ...
                <span>{t('consequences.delete_bill')}</span>
              </li>
              <li className="flex items-start gap-2 text-sm text-slate-600">
                <span className="text-green-600 font-bold">✓</span>
