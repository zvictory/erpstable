...
import { warehouses, warehouseLocations, inventoryLayers, items } from '../db/schema/index';
...
async function seedTestData() {
  try {
    ...
    // 3. Create locations for MAIN warehouse
    console.log('📍 Creating 120 locations for MAIN warehouse...');
    const mainLocations = [];
    ...
    for (let i = 0; i < mainLocations.length; i += 100) {
      const batch = mainLocations.slice(i, i + 100);
      if (batch.length > 0) {
    ...
    const itemsList = await db.select({ id: items.id, name: items.name }).from(items).limit(10);

    if (itemsList.length > 0) {
      const inventoryToAdd = [];
      const baseDate = new Date(2025, 0, 1); // Jan 1, 2025

      // Add inventory for items to various locations
      for (let itemIdx = 0; itemIdx < Math.min(10, itemsList.length); itemIdx++) {
        const item = itemsList[itemIdx];

        // Get 3 random locations for this item
        const locationsForItem = mainLocations.slice(itemIdx * 10, itemIdx * 10 + 3);

        for (let locIdx = 0; locIdx < locationsForItem.length; locIdx++) {
          const loc = locationsForItem[locIdx];
          const mainLocationDb = await db.select().from(warehouseLocations)
            .where(eq(warehouseLocations.locationCode, loc.locationCode))
            .limit(1);

          if (mainLocationDb.length > 0) {
            inventoryToAdd.push({
              itemId: item.id,
              batchNumber: `SEED-${item.id}-${locIdx}`,
              initialQty: Math.floor(Math.random() * 200) + 100,
              remainingQty: Math.floor(Math.random() * 200) + 100,
              unitCost: Math.floor(Math.random() * 10000) + 1000, // cents
              warehouseId: main.id,
              locationId: mainLocationDb[0].id,
              isDepleted: false,
              receiveDate: new Date(baseDate.getTime() + itemIdx * 24 * 60 * 60 * 1000),
              version: 1,
            });
          }
        }
      ...
      if (inventoryToAdd.length > 0) {
        ...
        for (let i = 0; i < inventoryToAdd.length; i += 50) {
          const batch = inventoryToAdd.slice(i, i + 50);
          if (batch.length > 0) {
            await db.insert(inventoryLayers).values(batch as any);
          }
        }
    ...
    const wareCount = await db.select({ count: sql<number>`count(*)` }).from(warehouses);
    const locCount = await db.select({ count: sql<number>`count(*)` }).from(warehouseLocations);
    const invCount = await db.select({ count: sql<number>`count(*)` }).from(inventoryLayers).where(sql`warehouse_id IS NOT NULL`);
